  ...
  <div class="container">
    ...
    <form @submit.prevent="save" enctype="multipart/form-data" class="form-box">
      ...
      <div class="form-row">
        ...
        <input type="number" v-model.number="length" step="0.01" min="0" required />
      </div>
      <div class="form-row">
        <label>Stock:</label>
      ...
      <div class="form-row">
        ...
        <div v-if="pictureUrl" style="margin-top: 0.4rem;">
          <img :src="pictureUrl" style="max-height: 65px; border-radius: var(--border-radius); box-shadow: 0 0 0.4rem var(--accent-color);" />
        </div>
      </div>
...
<script>
import { fetchPoles, updatePole } from '@/api/api'
...
export default {
  ...
  methods: {
    async loadPole() {
      const poleId = this.$route.params.id
      try {
        const { data } = await fetchPoles()
        const found = data.find(p => p.id == poleId)
        if (found) {
        ...
        this.errorMessage = "Failed to load pole data."
      }
    },
    onFileChange(e) {
    ...
    async save() {
      ...
      try {
        await updatePole(poleId, formData)
        this.successMessage = "Pole updated successfully."
